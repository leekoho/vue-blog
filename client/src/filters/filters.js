/**
 * Created by leekoho on 2017/4/22.
 */
...
export default {
  markdown (str) {
    return marked(str || '')   // 修复报`replace' of undefined `的错误
  },
  getTitle (str) {
    if (str) {
  ...
  getExcerpt (str) {
    // 已 `<!-- more -->`为标识, 分割正文
    const beforeMoreTag = str.split(/<!--more-->/)[0]
    const afterTitleTag = beforeMoreTag.split(/---/)[1]
    if (beforeMoreTag && afterTitleTag) {
      return afterTitleTag
    }
    return null
  },
  getContent (str) {
